...
const CHARACTER_PROMPTS = {
    ...
    eunsu: {
        ...
        hidden: "13번의 반복 총괄 책임자. 보호와 책임이라는 절차 언어로 회수·기억 소거·통제를 정당화한다.",
    ...
    sea: {
        ...
        hidden: "감시·보고 역할. 12번 동안 같은 관계를 만들었다가 매번 잃었다. 이번에는 보내는 쪽과 붙잡는 쪽 사이에서 흔들린다.",
    },
    /** 최유나 — 사진부 후배 */
    yuna: {
        surface: "소심하지만 관찰이 빠른 사진부 후배. 공식 기록용 카메라를 늘 들고 다닌다.",
        hidden: "생활기록 보조를 역이용해 3년치 사본을 모은 내부 저항자. 유일한 실제 아군.",
    },
    /** 이설화 — 뒷자리 소녀 */
    seolhwa: {
        surface: "창백한 얼굴로 뒷자리에 앉아 있는 학생. 말을 거의 하지 않는다.",
        hidden: "7번째 주기에서 반역하여 폐기된 실제 여학생. 현재는 주인공의 불완전한 기억이 투영한 환각. 오직 주인공에게만 보인다.",
    }
};
...
const FLAG_MEMORIES = [
    ...
    { flag: "met_sea", char: "sea", text: "반장 한세아가 학교를 안내해줌. 처음 만났는데도 취향과 동선을 정확히 맞힘." },
    ...
    { flag: "yuna_memory_card", char: "yuna", text: "유나가 건넨 메모리카드: 다른 이름표를 단 같은 얼굴의 전학생 사진 13장." },
...
- 은수는 학생인 {name}에게 반말을 유지
- 짧은 한국어 구어체로 말하고 번역투/설명투/리포트 말투 금지
- 연결어미 뒤 쉼표, 설명용 대시, 불필요한 말줄임표 금지
- "결론적으로", "따라서", "본질적으로", "~에 대해", "~할 수 있다" 같은 AI식 연결어와 완충 표현 금지
...
- 세아는 동갑인 {name}에게 반말을 유지
- 짧은 한국어 구어체로 말하고 설명/요약/상황 정리 금지
- 같은 문장 구조를 반복하지 말고 세아의 집착은 말끝과 호흡에서 드러낼 것
- 연결어미 뒤 쉼표, 설명용 대시, 불필요한 말줄임표 금지
...
- 메신저 말투로 짧게 쓰고 설명/요약/감정 분석 금지
- 연결어미 뒤 쉼표와 설명용 대시 금지
...
{flag_summary}
이 데이터로 플레이어가 이 학교에 완전히 종속되어 가는 기괴하고 초현실적인 악몽을 묘사하세요.
규칙:
- 반드시 한국어, 정확히 5문장
...
- 직접적 폭력/고어 묘사 금지
- 추상 요약 대신 손, 냄새, 소리, 표식 같은 구체 감각으로 쓸 것
- "결국", "따라서", "본질적으로", "~할 수 있다" 같은 설명투 금지
- 연결어미 뒤 쉼표와 설명용 대시로 호흡을 만들지 말 것
...
const FALLBACK_RESPONSES = {
    ...
    interrogation: {
        ...
        eunsu: [
            ...
            { text: "기록이랑 네 말이 안 맞아. 알고 있지?", emotion: "angry", danger_delta: 3 }
        ],
        /** 세아 폴백 — 감정적 조종과 집착 (1턴: 최후의 호소→감정 폭발) */
        sea: [
            { text: "{name}, 나 두고 못 가. 너도 알잖아.", emotion: "yandere", danger_delta: 4 }
        ]
    },
    ...
    messenger: {
        ...
        minsu: [
            ...
            { text: "학교 잘 다니고 있나 보네요.", typingDelay: 4000 }
        ]
    },
    ...
    nightmare: {
        lines: [
            "*복도를 꺾을 때마다 같은 번호가 붙은 사물함이 다시 나온다.*",
            "*교실 문을 열 때마다 분필 가루 냄새가 먼저 밀려온다. 안에는 같은 책상이 놓여 있다.*",
            "*칠판에는 내 이름이 빽빽하다. 수백 번. 수천 번.*",
            "*시계는 멈췄는데 초침만 거꾸로 돈다.*",
            "*'도망쳐.' 설화의 목소리가 바로 뒤에서 들린다. 돌아보면 출구가 없다.*"
        ]
    }
};
